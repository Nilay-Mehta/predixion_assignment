...
from bot.models import Plan, ToolCall
...
def execute_plan(plan: Plan, registry: ToolRegistry, llm: LLMProvider) -> list[ToolCall]:
    ...
    llm_calls = 0
    execute_plan.cap_hit = False
    total_steps = len(plan.steps)
    executor_llm_cap = max(settings.max_llm_calls_per_run - 1, 0)
    ...
    for index, step in enumerate(plan.steps, start=1):
        ...
        if (
            ...
            and llm_calls < executor_llm_cap
        ):
